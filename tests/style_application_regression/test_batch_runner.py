...
def test_new_role_specs_parameter_does_not_break_existing_positional_callers():
    assert list(signature(process_single_file).parameters)[-2:] == ["model", "role_specs"]
    assert list(signature(run_batch_concurrent).parameters)[-3:] == [
        "max_workers", "on_file_complete", "role_specs"
    ]
    assert list(signature(run_batch_api).parameters)[-6:] == [
        "max_workers", "poll_interval", "on_file_complete", "on_batch_poll", "model", "role_specs"
    ]
